...
const StartProject = () => {
  ...
  // Using the start-project endpoint
  const API_URL = 'https://thecodecreater-backend.onrender.com/api/start-project';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.name || !formData.email || !formData.projectType || !formData.budget) {
        throw new Error('Name, email, project type, and budget are required');
      }
      ...
      // Submit to live backend server
      const response = await fetch('https://thecodecreater-backend.onrender.com/api/project-submit', {
        method: 'POST',
        headers: {
